...
describe('PATCH /todos/:id', () => {
  ...
  it('should cltodoItem.ear completedAt when todo is not completed', (done) => {
    var hexId = todos[1]._id.toHexString();
    text = 'Second todo item --updated';
    request(app)
      .patch(`/todos/${hexId}`)
      .send({
        completed: false,
      ...
      // .expect((res) => {
      //   expect(res.body.text).notToBe(todos[0].text);
      // })
      // .end((err, res) => {
      //   if(err) {
      //     return done(err);
